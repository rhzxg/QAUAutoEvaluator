    ...
    baseUrl = "http://jwglxt.qau.edu.cn";
    ...
    if (evaluationInfoPage.indexOf(url) != -1) {
        ...
        // /\'(.*)\'/.exec(c)[0].slice(1, -1)
        var dataListTable = document.getElementById("dataList"); // 获取表格元素
        var rows = dataListTable.getElementsByTagName("tr"); // 获取所有行
        for (var i = 1; i < rows.length; i++) { // 从第二行开始遍历
            var cells = rows[i].getElementsByTagName("td"); // 获取行中的所有单元格
            var isEvaluated = cells[5].textContent.trim(); // 获取"已评"单元格的文本内容并去除空白
            if (isEvaluated === "是") {
                continue;
            } else {
                var teacherPageList = [];
                for(var t_i=0;t_i<teacherTableList.length;t_i++){
                    var link = /\'(.*)\'/.exec(teacherTableList[t_i].getAttribute("href"))[0];
                    if (link.endsWith("&type=view")) {
                        link = link.slice(1, -11);
                    }
                    link = link.slice(1,-1);
                    teacherPageList.push(link)
                }
                for(var t_j = 0; t_j < teacherPageList.length; t_j++) {
                    var completeUrl = baseUrl + teacherPageList[t_j];
